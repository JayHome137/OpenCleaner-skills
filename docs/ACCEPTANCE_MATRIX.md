...
既有 1.0.0 名称同步提交 `9cf09d79533c10ba65a729120bc52bac753263d4` 曾在真实 macOS 与 Windows runner 上完成验证。本轮补齐目录浏览、持久保护、自定义扫描根、安装包视图、App 归属、运行态保护和项目产物扩展，并完成本地验证；Windows 公开入口保持关闭，历史 Windows runner 只作来源记录，不代表当前支持。
...
| 单元与安全回归 | 本地通过 | 100 个测试通过；新增覆盖目录浏览边界、私有设置、保护降权、自定义扫描根、安装包元数据、App 归属、SQLite WAL/SHM、打开文件、共享 App 身份、Git 检查点、多构建系统与既有全部安全回归。 |
...
| 目录浏览与本地设置 | 本地通过 | 登记根内支持逐层进入、返回、搜索、名称/大小/时间排序、大小筛选和多选保护；设置文件为 0600、状态目录为 0700，拒绝 symlink 和越界根。 |
| 安装包与 App 归属 | 本地通过 | DMG/PKG/ISO/XIP/ZIP 专项视图及 Bundle ID、显示名、容器、缓存、Application Support、登录项和后台项关系进入版本化 analysis。 |
| 扩展运行态保护 | 本地通过 | SQLite WAL/SHM、打开文件、共享 Bundle ID、多版本 App、持久保护路径/App 均在服务端执行前失败关闭。 |
...
| 报告内容与布局 | 本地通过 | 新交互报告完成 1440×900、390×844 浏览器验证；无页面横向位移、控件越界或控制台错误。目录浏览、保护列表、安装包视图和移动端 Top 5 均可见。 |
