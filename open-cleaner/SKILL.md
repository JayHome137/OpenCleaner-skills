...
- SQLite 主文件与 WAL/SHM、打开文件、共享 Bundle ID 和多版本 App 必须在策略层执行前重验；检测到活动或状态未知时失败关闭。
- 本地保护路径或 App 只能降低建议等级并阻止处置，不能授权路径；设置保存在私有状态目录并拒绝符号链接。
- 自定义扫描根只允许主目录或 `/Volumes` 下当前真实挂载卷中的目录，只扩大扫描和浏览范围，不扩大 Trash allowlist。
...
除非用户明确要求只读文件或需要保存副本，不得用静态 HTML 替代受控服务作为主要结果入口。受控页面应直接展示绿色、橙色、红色三组：绿色表示当前计划允许移入废纸篓，橙色表示由用户逐项决定且只有已授权目标显示复核按钮，红色表示不能直接删除。每个条目在折叠状态也要显示完整路径；当前可操作的绿色或橙色条目默认展开按钮。
...
受控模式保持以下顺序：首屏决策摘要（安全等级 / 当前可行动 / 完全不可行动）、受控会话提示、绿色橙色红色逐项操作、磁盘总览、所有者聚合、覆盖与 APFS 解释、Top 5、执行建议、权限遗漏、历史与长期建议。静态只读报告可以将完整明细折叠到证据区，但必须明确说明它没有文件操作能力。
