...
| HTML 报告 | 可用：桌面与移动布局、目录浏览器、保护列表、安装包专项视图、Top 5、分级卡片、权限遗漏和长期建议。 |
| 受控处置 | 可用：绿灯批量 Trash；受限黄灯经独立复核后 Trash；红灯只定位查看。 |
| 运行态保护 | 可用：SQLite sidecar、打开文件、共享 App 身份、多版本 App、活动或未知所有者状态均失败关闭。 |
| 项目生成目录 | 可选：覆盖 Swift、Node、Python、Go、Rust、Maven、Gradle 与 Xcode allowlist 产物；必须属于 Git 工作区、明确 ignored 且不含 tracked 内容。 |
